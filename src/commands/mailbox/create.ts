...
export async function runMailboxCreate(ctx: CommandContext, name: string): Promise<void> {
  ...
  await imap.connect();
  try {
    await imap.createMailbox(name);
    ctx.formatter.printSuccess(`Mailbox "${name}" created`);
  } finally {
